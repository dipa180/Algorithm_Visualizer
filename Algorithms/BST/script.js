...
//resize the window when window size is changed
const resize = () => {
  width = Math.max(100, window.innerWidth - 50);
  height = Math.max(100, window.innerHeight - 100);

  d3.select('svg')
    .attr('width', width)
    .attr('height', height);
  update(data, data, -1, -1); // Redraw the tree with new dimensions
};
